...
case "${1}" in
    "autonomous")
      ...
      echo "JSON_DEFAULT_DATA_TYPE=\"BLOB check (%s is json format oson (size limit 32m))\"" >> ${GITHUB_OUTPUT}
...
EOF
      ...
      export JDBC_USER="micronaut_data_${RUNID}"
      export JDBC_PASSWORD="Oracle_19_Password"
      echo "JSON_DEFAULT_DATA_TYPE=\"BLOB check (%s is json)\"" >> ${GITHUB_OUTPUT}
